...
@section('subtitle','検索結果')

@section('content')
<div class="products">
    @if(!$products->isEmpty())
        <div class="search_result_count">"{{$keyword}}"に関する検索ヒット数 {{$count}} 件</div>
        ...
        @endforeach
    @else
        <div class="search_not_result">"{{$keyword}}"に該当する商品は見つかりませんでした。</div>
    @endif
    ...
    <div class="page">
            @for($i = 0; $i <= $max_page+1; $i++)
                    @if($i==0)
                        ...
                        @else
                        <input type="button" class="front_page" onclick="location.href='{{route('shopping.search',['keyword'=>$keyword,'page_id'=>$now-1])}}'" value="前のページ">                
                        @endif
                    @elseif ($i == $now)
                    {{$now}}
                    @elseif($i == $max_page+1)
                        ...
                        @else
                         <input type="button" class="back_page" onclick="location.href='{{route('shopping.search',['keyword'=>$keyword,'page_id'=>$now+1])}}'" value="次のページ">
                        @endif
                    @else
                    <a class="not_page" href="{{route('shopping.search',['keyword'=>$keyword,'page_id'=>$i])}}">{{$i}}</a>
                    @endif
            @endfor
    </div>
    @endif
